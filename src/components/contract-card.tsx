...
export function ContractCard({
  ...
  const totalConsumption = projection?.monthlyBreakdown.reduce(
    (sum, entry) => sum + entry.consumption,
    0,
  );

  const totalCost = projection?.monthlyBreakdown.reduce(
              ...
              <TabsContent value="table">
                <div className="border rounded-md">
                  <Table>
                    ...
                    <TableBody>
                      {projection.monthlyBreakdown.map((e) => (
                        <TableRow
                          key={e.month}
                          className={
                            isCurrentMonth(e.month)
